...
package pkg24cardgame;
import java.lang.*;
import java.util.*;
import java.io.*;
public class Main {
                  // Avoids floating-point trouble.
  int num, denom;
  ...
  Main(int num, int denom) {  // Makes a simplified fraction.
    if (denom == 0) {             // Division by zero results in
      this.num = this.denom = 0;  //  the fraction 0/0. We do not
    } else {                      //  throw an error.
      int x = Main.gcd(num, denom);
      this.num = num / x;
      this.denom = denom / x;     
    }
  }
  Main plus(Main other) {
    return new Main(this.num * other.denom + other.num * this.denom,
        this.denom * other.denom);
  }
  Main minus(Main other) {
    return this.plus(new Main(-other.num, other.denom));
  }
  Main times(Main other) {
    return new Main(this.num * other.num, this.denom * other.denom);
  }
  Main divide(Main other) {
    return new Main(this.num * other.denom, this.denom * other.num);
  }
  @Override
...
class Expression {                // A tree node containing a value and
  Main value;                 //  optionally an operator and its
